...
void PoseGraph::savePoseGraph()
{
    ...
    // 遍历关键帧列表
    list<KeyFrame *>::iterator it;
    std::cout << "keyframelist.size() " << keyframelist.size() << endl;
    for (it = keyframelist.begin(); it != keyframelist.end(); it++)
    {
        // 初始化文件路径
        ...
        // 保存关键点和描述符
        // 创建两个文件：一个用于保存BRIEF描述符（以二进制格式），另一个用于保存关键点坐标（以文本格式）
